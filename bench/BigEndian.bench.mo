...
module {
    public func init() : Bench.Bench {
        ...
        bench.name("ByteUtils library Benchmarks: Big Endian Conversions");
        ...
        for (i in Nat.rangeInclusive(0, 9_999)) {
            List.add(nat8Bytes, ByteUtils.BigEndian.fromNat8(fuzz.nat8.random()));
            List.add(nat16Bytes, ByteUtils.BigEndian.fromNat16(fuzz.nat16.random()));
            List.add(nat32Bytes, ByteUtils.BigEndian.fromNat32(fuzz.nat32.random()));
            List.add(nat64Bytes, ByteUtils.BigEndian.fromNat64(fuzz.nat64.random()));
            List.add(floatBytes, ByteUtils.BigEndian.fromFloat(fuzz.float.random()));
            List.add(int8Bytes, ByteUtils.BigEndian.fromInt8(fuzz.int8.random()));
            List.add(int16Bytes, ByteUtils.BigEndian.fromInt16(fuzz.int16.random()));
            List.add(int32Bytes, ByteUtils.BigEndian.fromInt32(fuzz.int32.random()));
            List.add(int64Bytes, ByteUtils.BigEndian.fromInt64(fuzz.int64.random()));
